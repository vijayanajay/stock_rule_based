        ...
        if reportable_open_positions:
            ...
            for pos in reportable_open_positions:
                ...
                open_pos_table += (
                    ...
                    f"{nifty_return_pct_str} | {pos['days_held']} / {config.hold_period} |\n"
                )
...
class Reporter:
    """Performance reporting with Rich terminal output."""
    ...
    def generate_performance_report(self, results: Dict[str, Any]) -> None:
        ...
        Refactored for H-9 compliance (was 45 lines, now split).
        """
        if not results:
            self.console.print("[yellow]No results to display[/yellow]")
            return
            
        self._print_summary_metrics(results)
        self._print_trade_details(results)
        self._print_risk_metrics(results)
    
    def _print_summary_metrics(self, results: Dict[str, Any]) -> None:
    ...
    def _print_trade_details(self, results: Dict[str, Any]) -> None:
        ...
        trade_table.add_column("Entry Date", style="yellow")
        trade_table.add_column("Exit Date", style="yellow")
        trade_table.add_column("Return", style="green")
        
        # Show last 10 trades
        for trade in trades[-10:]:
            return_style = "green" if trade.get('return', 0) > 0 else "red"
            trade_table.add_row(
                trade.get('ticker', ''),
                trade.get('entry_date', ''),
                trade.get('exit_date', ''),
                f"{trade.get('return', 0):.2%}",
                style=return_style
            )
        
        self.console.print(trade_table)
    
    def _print_risk_metrics(self, results: Dict[str, Any]) -> None:
        """Print risk analysis metrics."""
        risk_data = results.get('risk_metrics', {})
        if not risk_data:
            return
            
        panel_content = "\n".join([
            f"Volatility: {risk_data.get('volatility', 0):.2%}",
            f"VaR (95%): {risk_data.get('var_95', 0):.2%}",
            f"Beta: {risk_data.get('beta', 0):.2f}"
        ])
        
        panel = Panel(panel_content, title="Risk Metrics", border_style="red")
