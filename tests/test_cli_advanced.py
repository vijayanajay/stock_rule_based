...
def test_run_command_backtest_generic_exception_verbose(
    ...
    with runner.isolated_filesystem() as fs:
        ...
        assert result.exit_code == 1
        assert "An unexpected error occurred during run pipeline: Generic backtest error" in result.stdout
...
def test_analyze_strategies_command_success(mock_format_csv, mock_analyze, sample_config):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        assert "Analyzing aggregated strategy performance" in result.stdout
        ...
        mock_analyze.assert_called_once_with(db_path, min_trades=10)
...
def test_analyze_strategies_command_custom_output(mock_get_connection, mock_analyze, sample_config):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        # Story 22 change: Default is now aggregated format (logic inverted)
        mock_analyze.return_value = [{
            'strategy_rule_stack': 'test_strategy',
            'frequency': 1,
            'avg_edge_score': 0.5,
